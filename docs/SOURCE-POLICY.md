...
> **Status 0.3.0:** Voor 0.3.0 is `sources/registry.json` leidend. `retrieved_not_activated` betekent opgehaald maar niet juridisch goedgekeurd.
...
Alleen `active_reviewed` mag standaard in productieretrieval.
...
Nooit rechtstreeks `discover -> active_reviewed`.
